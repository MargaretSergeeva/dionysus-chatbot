...
--
-- Use for list/filter-style questions ("which hotels are pet-friendly?") where
-- semantic search over rheingau_rag_chunks_v2 cannot guarantee a complete,
-- exact-match result set.
...
LANGUAGE sql STABLE AS $$
  SELECT
    rp.page_id, rp.title, rp.category, rp.page_type, rp.source_url,
...
COMMENT ON FUNCTION public.filter_rheingau_pages IS
  'Structured-filter retrieval (DC2-131), companion to match_rheingau_chunks (DC2-119). Every boolean
   param left NULL is not filtered on; a true/false param only matches confirmed rows, never rows
   where the flag itself is NULL (unknown). Use for list/filter-style questions ("which hotels are
   pet-friendly?") where RAG cannot guarantee a complete result set.';
